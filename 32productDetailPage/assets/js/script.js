...
document.addEventListener("DOMContentLoaded", () => {
    ...
    //  Login olmadiqda wishlist sehifesi acilmamalidir login sehifesine kecid olmalidir.(Toast olaraq) 
    if (wishlistPage) {
    ...
    function createUserCard() {
        ...
        products.forEach((product) => {
            ...
            image.append(img);
            card.append(image, cardContent, cardFooter);
            card.append(heart, addBtn);
            cardsContainer.append(card);
        });

    }
    function checkAndClearSelectionsOnLogout() {
        if (!isLogineduser) {
            localStorage.removeItem("wishlist");
            localStorage.removeItem("cart");
        }
    }
    // checkAndClearSelectionsOnLogout();
